---
title: Arsitektur Pengetahuan (Knowledge)
description: Graf relasional pengetahuan, peta dependensi konsep, dan diagnostic questions.
status: active
...
3. Fondasi Universal (Prasyarat Lintas Peran)
...
15. Titik Konvergensi Lintas Track
...
Pada titik-titik ini, kolaborasi yang sukses menuntut pemahaman lintas peran:
...
- **Komunikasi teknis tertulis:** README (Dev), Handoff (UI/UX), PRD (PM), Test Report (QA), ADR (Software Dev), Data Story (Data & AI).
- **Problem decomposition:** Memecah masalah besar  -  prasyarat utama untuk naik ke Lapisan 3 di semua track.
- **Iterative delivery:** Rilis bertahap  -  Sprint (Agile), Canary Release (DevOps), A/B Test (Digital Product).
- **Feedback literacy:** Code Review (Dev), Design Critique (UI/UX), Stakeholder Feedback (PM), QA Sign-off (QA).
- **Knowledge documentation:** ADR (Dev), Decision Log (PM), Test Coverage Report (QA), Model Card (Data & AI).
- **Version control mindset:** Git (Dev/DevOps), Figma History (UI/UX), PRD Changelog (PM), Model Registry (Data & AI), Creative Asset Versioning (Creative Tech).

---
## 17. Cara Mentor Menggunakan Dokumen Ini
- **Untuk diagnosis gap**: Gunakan *Diagnostic Questions* berbasis skenario untuk menguji model mental member  -  bukan hafalan definisi. Jika member kesulitan menjawab pertanyaan di Lapisan 2 (meskipun tugasnya selesai), jangan berikan sign-off untuk Lapisan 3.
- **Untuk merancang coaching session**: Identifikasi konsep mana yang menjadi bottleneck. Gunakan GROW Model untuk memandu member merancang rencana perbaikan mandiri.
- **Untuk cross-track mentoring**: Gunakan Bab 15 (Titik Konvergensi) untuk membantu member memahami bagaimana peran mereka berhubungan dengan peran lain dalam proyek kolaboratif.
...
*Owner: Ghaniyyir Rahman Sudarsono (Founder YYZU) · Approved by: Ghaniyyir Rahman Sudarsono · Review: Tahunan · Status: v3.0 - Active*