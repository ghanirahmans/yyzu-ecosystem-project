...
Dokumen ini mengisi gap antara kompetensi (didefinisikan di Kurikulum) dan keputusan sign-off (dilakukan oleh Mentor). Ini adalah operasionalisasi dari **Prinsip 2** (Bukti Mendahului Klaim) dan **Prinsip 6** (Progression melalui Demonstrasi) dari Prinsip Pembelajaran YYZU.
...
| --- | --- |
| Web Development | Arsitektur atau implementasi menunjukkan miskonsepsi fundamental yang membuat sistem tidak memenuhi prinsip dasar keamanan, maintainability, atau correctness. *(Contoh: logika autentikasi yang sistematis tidak aman, data dikirim ke client tanpa validasi, skema yang tidak dapat berkembang — lihat Appendix untuk definisi operasional tiap contoh)* |
...
Sebelum mengajukan sign-off ke Mentor, member wajib mengisi self-assessment checklist:
...
- [ ] Saya dapat menjelaskan setiap keputusan signifikan yang saya ambil dalam artefak ini.
- [ ] Artefak ini dapat digunakan atau dipahami oleh orang lain tanpa penjelasan lisan dari saya.
- [ ] Seluruh **Exit Criteria** untuk level ini sudah saya periksa dan terpenuhi.
- [ ] Tidak ada *fundamental error* yang saya ketahui dan belum saya perbaiki.
...
Self-assessment ini dikirim ke Mentor **sebelum** sesi sign-off review.

### Langkah 2: Review oleh Mentor

Mentor membaca self-assessment dan mereview artefak secara asinkron menggunakan checklist yang relevan di dokumen ini. Mentor mempersiapkan clarifying questions seperlunya.

### Langkah 3: Sign-off Session (jika diperlukan)
...
**Langkah 1: Feedback tertulis dari Mentor** - Mentor memberikan SBIN feedback yang mengidentifikasi dimensi mana yang tidak terpenuhi, Exit Criteria mana yang belum terpenuhi, contoh konkret dari artefak, dan arah perbaikan yang spesifik (*Next* di SBIN).
...
**Langkah 3: Re-submission** - Member mengajukan ulang dengan self-assessment yang diperbarui, termasuk penjelasan eksplisit tentang apa yang berubah dari submission sebelumnya.
...
- Jika setelah tiga retry berturut-turut tidak ada progress yang terukur, Mentor dan SDM Management melakukan evaluasi bersama untuk menentukan langkah selanjutnya
...
2. Dokumen tersebut dikirim ke **Mentor Lead atau SDM Management** (bukan Mentor yang sama)
3. Mentor Lead atau SDM Management meninjau artefak dan feedback Mentor secara independen dan mengambil keputusan
4. Jika member masih tidak setuju dengan keputusan Mentor Lead / SDM Management, eskalasi dapat dilakukan ke **Founder** sebagai final escalation — khusus untuk kasus yang melibatkan inkonsistensi standar atau gap rubrik yang sistemik
...
**Calibration session:** Dilakukan minimal sekali per batch, sebelum batch dimulai. Mentor Lead memfasilitasi review bersama terhadap 2 - 3 contoh artefak - satu yang jelas memenuhi, satu yang jelas tidak memenuhi, dan satu yang borderline. Semua Mentor membuat keputusan independen terlebih dahulu, kemudian mendiskusikan perbedaan.

**Calibration log:** Setiap perbedaan signifikan dalam calibration session dicatat. Jika ada perbedaan yang berulang untuk dimensi yang sama, **Mentor Lead mengusulkan pembaruan rubrik dan Founder menyetujui sebelum perubahan berlaku resmi**.
...
| --- | --- | --- | --- |
| 1.1 | Juni 2026 | Remediation audit: Kompetensi Lintas Track dibuat track-agnostic; gating Dimensi 1 direvisi ke evaluasi penuh dengan auto-NOT YET; retry mechanism dilonggarkan untuk revisi signifikan; appeal mechanism diperjelas dengan tiered escalation; ownership update rubrik ditambahkan; rubrik Fundamental Error, CI/CD, dan commit history diperjelas | Ghaniyyir Rahman Sudarsono (Founder YYZU) |